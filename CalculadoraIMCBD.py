...
def inserir_dados_no_banco(nome, peso, altura, valor_imc):
    conexao_str = "DRIVER={SQL Server};SERVER=RAFAEL_VIEIRA\\SQLEXPRESS;DATABASE=IMC;Trusted_Connection=yes;"
    conexao = pyodbc.connect(conexao_str)
    cursor = conexao.cursor()

    # Inserir dados na tabela
    cursor.execute("INSERT INTO TabelaUsuario (Nome_usuario, Peso, Altura, Valor_IMC) VALUES (?, ?, ?, ?)",
                   nome, peso, altura, valor_imc)
...
def calcular_imc():
    try:
        ...
        inserir_dados_no_banco(nome, peso, altura, imc)
